...
class LocalPlanner(object):
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None):
        ...
        self._args_lateral_dict = {'K_V': 0, 'K_S': 0, 'dt': 0}
        self._args_longitudinal_dict = {'K_P': 0, 'K_I': 0, 'K_D': 0, 'dt': 0}
    ...
    def set_overtake_plan(self, overtake_plan, overtake_distance):
        end_overtake_wp = list(self._waypoints_queue[0][0].next(overtake_distance))

        if len(end_overtake_wp) == 0:
            return list(self._waypoints_queue)
        else:
            end_overtake_wp = end_overtake_wp[0]

        plan_wps = [wp[0] for wp in self._waypoints_queue]

        i = 0

        while (i < len(plan_wps) and plan_wps[i].transform.location.distance(
                end_overtake_wp.transform.location) > self._sampling_radius):
            i += 1
        while (i < len(plan_wps) and plan_wps[i].transform.location.distance(
                end_overtake_wp.transform.location) < self._sampling_radius):
            i += 1

        overtake_plan.extend(list(self._waypoints_queue)[i:])
